...
require_once('../../funciones/sesiones.php');
	 ...
	 */
	$id=$_SESSION["usuario"];
	$aColumns = array( "id_mensajes_enviados","asunto","fecha","usuario_emisor","usuario_receptor","id_usuario_emisor","id_usuario_receptor");
	
	/* Indexed column (used for fast and accurate table cardinality) */
	$sIndexColumn = "id_mensajes_enviados";
	
	/* DB table to use */
	$sTable = "mensajes_enviados";
	 ...
	 */
	$sWhere = "WHERE id_usuario_emisor='".$_SESSION['id_usuario']."'";
	...
	for ( $i=0 ; $i<count($aColumns) ; $i++ )
	{
		if ( isset($_GET['bSearchable_'.$i]) && $_GET['bSearchable_'.$i] == "true" && $_GET['sSearch_'.$i] != '' )
		{
			if ( $sWhere == "" )
			{
				$sWhere = "WHERE receptor =".$_SESSION['id_usuario'];
			}
			else
			{
				$sWhere .= " AND ";
